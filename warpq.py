...
parser.add_argument(
    "--csv_input",
    type=str,  # default='./audio_paths.csv',
...
parser.add_argument(
    "--org", type=str, help="Path of the original (reference) speech file"
)
parser.add_argument(
    "--deg", type=str, help="Path of the degraded (processed) speech file"
)
parser.add_argument(
    "--sr",
    type=int,
    default=16000,
    help="Sampling frequency of speech signals in Hz. Only two sr values are currently supported: 16000 and 8000 Hz",
)
parser.add_argument("--n_mfcc", type=int, default=13, help="Number of MFCCs")
parser.add_argument(
    "--fmax", type=int, default=5000, help="Cutoff frequency for MFCC in Hz"
)
parser.add_argument(
    "--patch_size", type=float, default=0.4, help="Size of MFCC patch in seconds"
)
parser.add_argument(
    "--sigma",
    type=list,
    default=[[1, 0], [0, 3], [1, 3]],
    help="Step size conditon for DTW",
)
parser.add_argument(
    "--apply_vad", type=bool, default=True, help="Condition for using vad algorithm"
)
parser.add_argument(
    "--mapping_model",
    type=str,
    required=True,
    help="File name of pretrained model to map raw WARP-Q scores onto MOS",
)
parser.add_argument(
    "--csv_output", type=str, help="Path and name of a csv file to save WARP-Q results"
)
parser.add_argument(
    "--getPlots",
    type=bool,
    default=True,
    help="To plot the predicted scores vs MOS. If True, MOS and Codec type should be provided in the input csv file",
)
...
if args["mode"] == "predict_csv":
    ...
    if args["org"] is None:
        raise ValueError(
            "--org argument with path to input original speech file is required"
        )
    if args["deg"] is None:
        raise ValueError(
            "--deg argument with path to input degraded speech file is required"
        )
...
def main(args):
    ...
    if args["mode"] == "predict_csv":
        ...
        for index, row in tqdm(
            df.iterrows(), total=df.shape[0], desc="Compute quality sores..."
        ):

            rawScore, mappedScore = warpq.evaluate(
                ref_path=row["Ref_Wave"], test_path=row["Test_Wave"]
            )
        ...
        if args["getPlots"]:
            ...
            sns.relplot(
                x="MOS", y="Raw WARP-Q Score", hue="Codec", palette="muted", data=df
            ).fig.suptitle(
                "Correlations: Pearsonr= "
                + str(round(pearson_coef, 2))
                + ", Spearman="
                + str(round(Spearmanr_coef, 2))
            )
            ...
            sns.relplot(
                x="MOS", y="Mapped WARP-Q Score", hue="Codec", palette="muted", data=df
            ).fig.suptitle(
                "Correlations: Pearsonr= "
                + str(round(pearson_coef, 2))
                + ", Spearman="
        ...
        print(
            "\nRaw WARP-Q score (lower rating means better quality): "
            + str(warpq_rawScore)
        )
        print(
            "Mapped WARP-Q score (higher rating means better quality): "
            + str(warpq_mappedScore)
        )
